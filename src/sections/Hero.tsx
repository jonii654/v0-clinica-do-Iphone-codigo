...
// ─── Apple logo (simplified bezier shape) ────────────────────────
...
function IPhoneModel() {
  ...
  const backTexture = useMemo(() => {
    ...
    g.addColorStop(0.3, '#1e1e22');
    g.addColorStop(0.7, '#1a1a1e');
...
export function Hero() {
  ...
  const marqueeText = 'QUALIDADE E AQUI';
  const repeatCount = 8;
  ...
  return (
    <section className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-white">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full opacity-[0.035]"
          style={{ background: 'radial-gradient(circle, #1E3A8A 0%, transparent 70%)' }}
        />
      </div>

      {/* Scrolling marquee text behind iPhone - like a train running behind */}
      <div className="absolute inset-0 flex items-center pointer-events-none select-none z-[1] overflow-hidden">
        <div
          className="flex whitespace-nowrap"
          style={{
            animation: 'facade-scroll 12s linear infinite',
          }}
        >
          {Array.from({ length: repeatCount }).map((_, i) => (
            <span
              key={i}
              className="text-[18vw] md:text-[12vw] lg:text-[10vw] font-black tracking-tighter mx-6 md:mx-10"
              style={{
                color: 'transparent',
                WebkitTextStroke: '1.5px rgba(30, 58, 138, 0.12)',
              }}
            >
              {marqueeText}
            </span>
      ...
      {/* Second row of marquee (opposite direction) */}
      <div className="absolute inset-0 flex items-center pointer-events-none select-none z-[1] overflow-hidden" style={{ transform: 'translateY(12vh)' }}>
        <div
          className="flex whitespace-nowrap"
          style={{
            animation: 'facade-scroll-reverse 14s linear infinite',
          }}
        >
          {Array.from({ length: repeatCount }).map((_, i) => (
            <span
              key={i}
              className="text-[14vw] md:text-[9vw] lg:text-[7vw] font-black tracking-tighter mx-6 md:mx-10"
              style={{
      ...
      {/* Content layer - above the marquee */}
      <div className="relative z-10 w-full px-6 flex flex-col items-center">

        {/* Logo + Title */}
        <div ref={titleRef} className="flex flex-col items-center mb-2" style={{ opacity: 0 }}>
          <img
            src="/images/logo-clinica.jpg"
            alt="Clinica do iPhone"
            className="h-16 w-16 md:h-20 md:w-20 rounded-full object-cover mb-3 shadow-lg"
          />
